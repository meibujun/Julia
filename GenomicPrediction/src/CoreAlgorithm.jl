...
function gibbs_sampler!(model::BayesianModel, y::Vector, G::Matrix; rng::AbstractRNG)
    ...
    for iter in 1:model.iterations
        ...
        if iter > model.burn_in && (iter - model.burn_in) % model.thin == 0; push!(model.beta_samples, copy(beta)); end
...
mutable struct BayesAModel <: BayesianModel; nu_beta::Float64; s2_beta::Float64; nu_e::Float64; s2_e::Float64; iterations::Int; burn_in::Int; thin::Int; effects::Vector{Float64}; intercept::Float64; beta_samples::Vector{Vector{Float64}}; marker_vars::Vector{Float64}; BayesAModel(; nu_beta=5.0, s2_beta=0.01, nu_e=5.0, s2_e=0.1, iterations=2000, burn_in=500, thin=5) = new(nu_beta, s2_beta, nu_e, s2_e, iterations, burn_in, thin, [], 0.0, [], []); end
...
mutable struct BayesBModel <: BayesianModel; pi::Float64; nu_beta::Float64; s2_beta::Float64; nu_e::Float64; s2_e::Float64; iterations::Int; burn_in::Int; thin::Int; effects::Vector{Float64}; intercept::Float64; beta_samples::Vector{Vector{Float64}}; BayesBModel(; pi=0.95, nu_beta=5.0, s2_beta=0.01, nu_e=5.0, s2_e=0.1, iterations=2000, burn_in=500, thin=5) = new(pi, nu_beta, s2_beta, nu_e, s2_e, iterations, burn_in, thin, [], 0.0, []); end
...
mutable struct BayesCModel <: BayesianModel; pi::Float64; nu_g::Float64; s2_g::Float64; nu_e::Float64; s2_e::Float64; iterations::Int; burn_in::Int; thin::Int; effects::Vector{Float64}; intercept::Float64; beta_samples::Vector{Vector{Float64}}; marker_in_model::Vector{Bool}; BayesCModel(; pi=0.95, nu_g=5.0, s2_g=0.01, nu_e=5.0, s2_e=0.1, iterations=2000, burn_in=500, thin=5) = new(pi, nu_g, s2_g, nu_e, s2_e, iterations, burn_in, thin, [], 0.0, [], []); end
init_genetic_variance(model::BayesCModel, v) = model.s2_g
function sample_marker_variance(model::BayesCModel, j::Int, beta, var_g, rng); log_p1 = log(1.0 - model.pi) + logpdf(Normal(0, sqrt(var_g)), beta[j]); log_p0 = log(model.pi) + logpdf(Normal(0, sqrt(var_g * 1e-6)), beta[j]); prob_in = exp(log_p1) / (exp(log_p1) + exp(log_p0)); model.marker_in_model[j] = rand(rng) < prob_in; return model.marker_in_model[j] ? var_g : 1e-12; end
function sample_genetic_variance(model::BayesCModel, beta, var_g, rng); beta_in_model = beta[model.marker_in_model]; num_in_model = length(beta_in_model); shape = model.nu_g / 2 + num_in_model / 2; scale = (dot(beta_in_model, beta_in_model) + model.nu_g * model.s2_g) / 2; return rand(rng, InverseGamma(shape, scale)); end
mutable struct BayesRModel <: BayesianModel; mixture_pis::Vector{Float64}; mixture_vars::Vector{Float64}; nu_e::Float64; s2_e::Float64; iterations::Int; burn_in::Int; thin::Int; effects::Vector{Float64}; intercept::Float64; beta_samples::Vector{Vector{Float64}}; BayesRModel(; mixture_pis=[0.5, 0.2, 0.2, 0.1], mixture_vars=[0.0, 0.0001, 0.001, 0.01], nu_e=5.0, s2_e=0.1, iterations=2000, burn_in=500, thin=5) = new(mixture_pis, mixture_vars, nu_e, s2_e, iterations, burn_in, thin, [], 0.0, []); end
