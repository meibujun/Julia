...
@testset "核心数据与关系矩阵测试" begin
    ...
    @testset "A 矩阵计算" begin
        dm.A_matrix, sorted_ped, animal_map = compute_A_matrix(dm.pedigree)
        dm.pedigree = sorted_ped # 更新为排序后的谱系
        dm.animal_map = animal_map
        dm.A_inv_matrix = compute_A_inv_matrix(dm.pedigree, dm.animal_map)
        ...
        # 验证 A * A⁻¹ ≈ I
        @test Matrix(dm.A_matrix * dm.A_inv_matrix) ≈ I(5) atol=1e-8
    end
