...
function run_evaluation(model::ModelSpec, dm::DataManager;
    ...
    all_animals = sort(collect(keys(dm.animal_map)))
    breeding_values_df = DataFrame(animal_id=all_animals, EBV=u)

    # 可靠性计算占位符
    reliability_df = DataFrame(animal_id=all_animals, reliability=zeros(length(u)))
