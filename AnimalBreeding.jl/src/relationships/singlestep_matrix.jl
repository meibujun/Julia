...
function compute_H_matrix_inv(dm::DataManager; blending_factor::Float64=0.05)
    ...
    perm = align_genotypes_to_pedigree!(dm)
    if !isempty(perm) && !isnothing(dm.G_matrix)
        dm.G_matrix = dm.G_matrix[perm, perm]
    end
    ...
    for (local_i, global_i) in enumerate(genotyped_idx)
        for (local_j, global_j) in enumerate(genotyped_idx)
            value = diff_matrix[local_i, local_j]
            if !iszero(value)
                push!(rows, global_i)
                push!(cols, global_j)
                push!(vals, value)
    ...
    delta = sparse(rows, cols, vals, size(H_inv, 1), size(H_inv, 2))
    H_inv = H_inv + delta
